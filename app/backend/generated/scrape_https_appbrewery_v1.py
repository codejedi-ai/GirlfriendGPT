"""Auto-generated tool from workflow scrape_https_appbrewery.
...
WORKFLOW_ID = 'scrape_https_appbrewery'
VERSION = 1
...
async def run(tools, params: dict[str, Any]) -> Any:
    ...
    args = _resolve_args({"listing": "{{ format_listing_2 }}"}, ctx)
    result = await tools.get('format_listing').fn(**args)
    ctx['format_listing_3'] = result
    args = _resolve_args({"listing": "{{ format_listing_3 }}"}, ctx)
    result = await tools.get('format_listing').fn(**args)
    ctx['format_listing_4'] = result
    args = _resolve_args({"listing": "{{ format_listing_4 }}"}, ctx)
    result = await tools.get('format_listing').fn(**args)
    ctx['format_listing_5'] = result
    args = _resolve_args({"listing": "{{ format_listing_5 }}"}, ctx)
    result = await tools.get('format_listing').fn(**args)
    ctx['format_listing_6'] = result
    args = _resolve_args({"listing": "{{ format_listing_6 }}"}, ctx)
    result = await tools.get('format_listing').fn(**args)
    ctx['format_listing_7'] = result

    return _resolve_template('{{ format_listing_7 }}', ctx)
